...
interface IndicatorSelectorProps {
  visible: boolean;
  onCancel: () => void;
  onSelect: (indicatorId: string, indicatorName: string, unit: string) => void;
}
...
const IndicatorSelector: React.FC<IndicatorSelectorProps> = ({
  ...
  const [selectedRow, setSelectedRow] = useState<IndicatorOption | null>(null);
  ...
  const columns = [
    ...
    { title: "数据来源方式", dataIndex: "dataSource", key: "dataSource" },
    ...
    { title: "指标归属", dataIndex: "department", key: "department" },
  ...
  const handleRowClick = (record: IndicatorOption) => {
    setSelectedRowKeys([record.id]);
    setSelectedRow(record);
  };

  // 修改 IndicatorSelector 组件，确保选择后正确创建指标元素
  const handleConfirm = () => {
    if (selectedRow) {
      onSelect(selectedRow.id, selectedRow.name, selectedRow.unit);
    }
  };
  ...
  const rowSelection = {
    ...
    onChange: (keys: React.Key[]) => {
      ...
      const selected = indicatorOptions.find((item) => item.id === keys[0]);
      if (selected) {
